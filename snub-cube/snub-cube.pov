/*
This program generates a snub cube.
*/ 
...
//selecting vertices to translate on each square face for the rhombicuboctahedron   
#declare i=0;
...
#while(i<24)  // vertices
    #declare j=0;
    #while (j<6) // centers
        #if ( vdot(Rho[i],Center[j])<(0.6036+0.1) & vdot(Rho[i], Center[j] )> (0.6036-0.1) )  
            #declare Snubcu[konter3]= vaxis_rotate(Rho[i],     Center[j], angulo) ;
            sphere {Snubcu[konter3], 0.1 texture {pigment{color Red}} finish{phong 1}}
...
#warning concat(str(konter3,2,0))
...
#while (i<24)
    #declare j=i+1;
    #while (j<24)
        #declare Dist=VDist(Snubcu[i],Snubcu[j]);
        #if (Dist<=L)
            cylinder{Snubcu[i],Snubcu[j], 0.03 texture {pigment{color Green}} finish{phong 1}}
        #end
        #declare j=j+1;
    #end
    #declare i=i+1;
